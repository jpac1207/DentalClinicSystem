...
namespace HealthSolution.Controllers
{
    ...
    public class IntervencaoViewModelController : Controller
    {
        ...
        public ActionResult ExportAll([Form] QueryOptions queryOptions, string doutor, string paciente, string procedimento, string data)
        {
            try
            {
                var intervencaoViewModels = new List<IntervencaoViewModel>();
                ...
                StringWriter sw = new StringWriter();
                HtmlTextWriter htw = new HtmlTextWriter(sw);
                string fileName = "Export_Procedimentos_Completo_" + DateTime.Now.ToString("dd.MM.yyyy") + ".xls";
